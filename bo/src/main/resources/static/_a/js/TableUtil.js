let GRID = {
    ...
    set: function(obj) {
		...
		if(this.area != undefined){
			let a = document.getElementById(this.area);
			a.id = this.id;
		}
        ...
        table += "<div class='gridBtnBx' id='"+this.id + "_btnBx'></div></div>";
        ...
        if (this.excel != "") {
            ...
            button.className = "btn_excel";
            //console.log(document.getElementById(this.id + "_excelBx"));
            document.getElementById(this.id + "_btnBx").append(button);
                ...
                for (j in bodyCol) {
                    ...
                    if (bodyCol[j].button != undefined) {
                        for (key in bodyCol[j].button) {
                            tag += "<a href='javascript:void(0)' class='btn_c' onclick=" + bodyCol[j].button[key] + "(" + i + ");>" + key + "</a>";
                        }
    ...
    setTableBtn: function() {
        ...
        if (this.opts.tableBtn != undefined) {
            let btnBx = document.getElementById(this.id + "_btnBx");
            let i = 0;
            for (key in this.opts.tableBtn.button) {
                let b = document.createElement("a");
                b.className = "btn_c gridBtn";
                b.id = this.id+ "_tableBtn_" + i++;
                b.text = key;
                btnBx.append(b);
			...
			let btn = btnBx.querySelectorAll(".gridBtn");
			for (let i = 0; i < btn.length; i++) {
            	btn[i].onclick = function() {
					if (_this.opts.tableBtn.button[this.text].retFunc != undefined) {
                        _this.opts.tableBtn.button[this.text].retFunc();
                    }     
				}
        	}
